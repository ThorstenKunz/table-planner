...
def _format_member_entry(entry: object) -> str:
    """Prefer a stored Discord display name, then a plain user ID."""
...
def create_table_embed(table_data: TableData, table_id: str) -> discord.Embed:
    ...
    gm_display_name = table_data.get("gm_display_name")
    if isinstance(gm_display_name, str) and gm_display_name.strip():
        safe_gm_name = escape_mentions(escape_markdown(gm_display_name.strip()))
    else:
        safe_gm_name = str(get_gm_id(table_data))
    embed.add_field(name="🧙 GM", value=safe_gm_name, inline=True)
    embed.add_field(name="📜 Infos", value=safe_infos, inline=False)

    players = table_data["players"]
    player_mentions = [f"• {_format_member_entry(entry)}" for entry in players]
    ...
    waitlist_mentions = [f"• {_format_member_entry(entry)}" for entry in waitlist]
